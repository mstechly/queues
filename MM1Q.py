...
def QSim(lambda=False,mu=False,simulation_time=False):
    """
	...
	if not lambda:
		lambda=input('Inter arrival rate: ')
	if not mu:
...
#----------------------------------
	...
	if input("Output data to csv (True/False)? "):
		outfile=open('MM1Q-output-(%s,%s,%s).csv' %(lambda,mu,simulation_time),'wb')
